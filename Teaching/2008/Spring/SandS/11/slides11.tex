...
\section[Regulære udtryk]{En big-step operationel semantik for
  regulære udtryk}

\begin{frame}[t]
  \alert{Regulære udtryk} over et givet alfabet $\Sigma$ :
  \begin{enumerate}
  \item abstrakt syntaks
    \begin{itemize}
    \item syntaktiske kategorier

      \qquad \blue{$a\in \Sigma$ -- tegn}
    \item opbygningsregler

      \qquad \blue{$\text{RE}_\Sigma\ni R \;::=\; a\mid \epsilon\mid
        \emptyset\mid R\cup R\mid R\circ R\mid R^*$}
    \end{itemize}

  \item semantiske mængder og hjælpefunktioner

    \qquad \blue{(har vi ikke her)}

  \item transitionssystem(er)
    \begin{itemize}
    \item konfigurationer og slutkonfigurationer

      \qquad \blue{$\Gamma= \text{RE}_\Sigma\cup \Pot( \Sigma^*)$}
    \item transitionsrelationen givet ved \emph{transitionsregler}

      \vspace*{-3ex}
      {\color{blue}
      \begin{gather*}
        a\to\{ a\}\qquad \epsilon\to\{ \epsilon\}\qquad \emptyset\to
        \emptyset \\[1ex]
        \frac{ R_1\to L_1\quad R_2\to L_2}{ R_1\cup R_2\to L_1\cup
          L_2}\qquad \frac{ R_1\to L_1\quad R_2\to L_2}{ R_1\circ
          R_2\to L_1\circ L_2}\qquad \frac{ R\to L}{ R^*\to L^*}
      \end{gather*}}
    \end{itemize}
  \end{enumerate}
\end{frame}
