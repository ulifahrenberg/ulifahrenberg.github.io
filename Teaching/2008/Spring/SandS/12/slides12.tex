...
\begin{frame}
  ...
  \item \alert{dynamisk binding af variable og procedurer:} $\texttt y=
    10$
  \item \alert{statisk binding af variable og procedurer:} $\texttt y=
    9$ \blue{(hint: det er et andet \texttt x !)}
...
\begin{frame}[t]
  ...
  \item \alert{reference}parametre: den \alert{formelle} parameter $x$ er
    ...
    parameter $y$
  \item (klares ved pointers i \texttt C)
...
\begin{frame}
  ...
  \bigskip (kan også klares ved at modificere \runa{proc}{bss} i
  stedet \blue{(hvordan?)})
\end{frame}
