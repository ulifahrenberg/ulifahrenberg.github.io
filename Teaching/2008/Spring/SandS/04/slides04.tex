...
\begin{frame}
  ...
  Et tip / ønske til syntaksopgaven:

  \medskip Indfør 4 alfabeter:
  \begin{align*}
    \Sigma_0 &= \{ \texttt0\} \\
  ...
  \end{align*}
  Sæt $\Sigma= \Sigma_0\cup \Sigma_1\cup \Sigma_2\cup \Sigma_3$, og
  betragt alle automater og udtryk \alert{over alfabetet $\Sigma$}.

  \medskip Brug $\Sigma_0$, $\Sigma_1$, $\Sigma_2$, $\Sigma_3$ som
  forkortelser på automaters pile og i udtrykkene.
\end{frame}

\begin{frame}
  \frametitle{Planen}
  \begin{itemize}
  \item i dag: afslutning på kursusdelen omhandlende regulære sprog
  \item og afslutning på syntaksopgavens del omhandlende regulære
    sprog
  \item næste gang: perspektivering og \alert{spørgetime} !
  \item og start på \alert{kontekstfrie sprog}
  \end{itemize}
\end{frame}
...
\section[Eksempel]{Eksempel på delmængdekonstruktion}

\begin{frame}
  \frametitle{Opgave 1.16}

  Konvertér følgende to NFAs til DFAs ved hjælp af
  delmængdekonstruktionen: \quad\blue{(ved tavlen)}
  \begin{equation*}
...
\begin{frame}
  ...
  automater, både DFA og NFA:

  \medskip \alert{Definition:} Et \alert{transitionssystem} er en
  4-tupel \alert{$(Q, \Sigma, T, q_0)$}, hvor delene er
  \begin{enumerate}
  \item $Q$ : en mængde af tilstande \blue{(endelig eller uendelig)}
  \item $\Sigma$ : et alfabet \blue{(en endelig mængde)}
  ...
  \item En NFA er et \alert{endeligt} transitionssystem med en
    specificeret mængde af \alert{accepttilstande}, og med et specielt
    tegn $\alert\epsilon\in \Sigma$
  \item En DFA er en NFA som opfylder følgende egenskaber:
    \begin{enumerate}
    ...
    \item hvis $(q, a, q_1')\in T$ og $(q, a, q_2')\in T$, så er
      $q_1'= q_2'$
    \end{enumerate}
...
\begin{frame}[t]
  \begin{textblock*}{.4\textwidth}(.58\textwidth,.7\textheight)
    \only<2>{{\color{blue}%
      ...
      er $\mathcal R( \Sigma)$, så transitionerne er $T\subseteq
